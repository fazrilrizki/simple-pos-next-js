...
export const CreateOrderSheet = ({
  ...
  const  {mutate: createOrderSheet, data:createOrderResponse } = api.order.createOrder.useMutation({
    onSuccess: () => {
  ...
  const { mutate: checkOrderPaymentStatus, data: orderPaid, isPending: checkOrderPaymentStatusIsPending, reset: resetCheckOrderPaymentStatus } = api.order.checkOrderStatus.useMutation({
    onSuccess: (orderPaid) => {
      if (orderPaid) {
        cartStore.clearCart();
      }
